...
rib1 = Electrode('ribbon', ribbon_angle=5.3)
rib2 = Electrode('ribbon', ribbon_angle=6.9)
rib3 = Electrode('ribbon', ribbon_angle=11.9)
...
potentials= np.array([  0,     0, -10, 100,  400,  700,  1000])*1000 # Volts

dz=0.01
domain = Domain(electrodes, potentials, positions, dz=dz, SI=True)

#%% Test particle
a0 = 5 # degrees, the beam angle
W0 = 0.85e6*2 * e # Twice the max grid voltage bc Z=2 for alphas
...
Hepart = super_maxwellian(100, 3.5e10, yrange, zrange, kT, \
        uz=3.9e6, mass=4, q=2, theta=a0, ymax=ymax, SI=True, beam=True)
